process rmats_prep {
  ...
  conda "bioconda::rmats==4.3.0"
  container "${ workflow.containerEngine == 'singularity' && !task.ext.singularity_pull_docker_container ?
      'docker://xinglab/rmats:v4.3.0' :
      'docker://xinglab/rmats:v4.3.0' }"

  publishDir path: "${params.outdir}/splicing_analysis", mode: 'copy', pattern: "prep_*_read_outcomes_by_bam.txt"
  ...
  tuple val(bam_i),
        path("outfd/*.rmats"),
        path("prep_${group_id}_*_read_outcomes_by_bam.txt"),
        val(bam_name)

  script:
  def prefix = task.ext.prefix ?: "${meta.id}"

  bam_id = "${group_id}_${prefix}"
...
process rmats_post {
  ...
  publishDir "${params.outdir}/splicing_analysis", mode: 'copy'
  ...
  python /rmats/rmats.py \\
    ...
    tar -czf ${params.outdir}.tar.gz *.txt
  """
}
